-- Prefer locale search rows; fall back to EN when that locale row is missing.
  ...
  for r in
    ...
    where (
        ...
        or (
          ...
          and not exists (
            select 1
            from public.search s2
            where s2.internal_id = s.internal_id
              and s2.locale = v_locale
          )
        )
      )
      and (
    ...
    order by
      case when s.locale = v_locale then 1 else 0 end desc,
      case
...
as $$
  ...
  scored as (
    select
      ...
      greatest(
        ...
        public.title_word_similarity(c.name, q.raw)
      )::real as score
    from candidates c
    cross join q
  )
  ...
  order by
    case when scored.locale = q.locale then 1 else 0 end desc,
    scored.score desc,
    scored.release_date desc nulls last
  limit 1;
$$;
