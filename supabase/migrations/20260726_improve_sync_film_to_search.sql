-- Improve EN search document rebuild for films:
-- - dedupe creators
-- - strip VTT chrome before indexing transcripts
-- - include note, location, rating, theme in search_content
-- - re-sync when creator_map / transcript change

create or replace function public.clean_vtt(raw text)
returns text
language sql
immutable
as $$
  select nullif(
    trim(
      regexp_replace(
        regexp_replace(
          regexp_replace(
            regexp_replace(
              coalesce(raw, ''),
              'WEBVTT',
              '',
              'gi'
            ),
            '\d{2}:\d{2}:\d{2}[.,]\d{3}\s*-->\s*\d{2}:\d{2}:\d{2}[.,]\d{3}',
            ' ',
            'g'
          ),
          '(^|\n)\s*\d+\s*(\n|$)',
          ' ',
          'g'
        ),
        '\s+',
        ' ',
        'g'
      )
    ),
    ''
  );
$$;

create or replace function public.sync_film_to_search(p_film_id uuid)
returns void
language plpgsql
security definer
set search_path = public
as $$
declare
  v_film record;
  v_creators text;
  v_theme_name text;
  v_rating_name text;
  v_tags text;
  v_transcript text;
  v_location text;
  v_combined_content text;
begin
  select f.*
  into v_film
  from public.film f
  where f.id = p_film_id;

  if not found then
    delete from public.search where internal_id = p_film_id and item_type = 'film';
    return;
  end if;

  select t.name into v_theme_name
  from public.theme t
  where t.id = v_film.theme
  limit 1;

  select r.name into v_rating_name
  from public.rating r
  where r.id = v_film.rating
  limit 1;

  -- Distinct creator names, ordered by first appearance
  select string_agg(name, ', ' order by min_sort)
  into v_creators
  from (
    select c.name, min(cm.sort_order) as min_sort
    from public.creator_map cm
    join public.creator c on c.id = cm.creator_id
    where cm.film_id = p_film_id
      and c.name is not null
      and length(trim(c.name)) > 0
    group by c.name
  ) creators;

  select string_agg(t.name, ' ' order by t.name)
  into v_tags
  from public.tag_map tm
  join public.tag t on t.id = tm.tag_id
  where tm.film_id = p_film_id
    and t.name is not null;

  select left(public.clean_vtt(tr.content), 12000)
  into v_transcript
  from public.transcript tr
  where tr.film_id = p_film_id
    and tr.language_code = 'en'
  order by length(coalesce(tr.content, '')) desc
  limit 1;

  v_location := nullif(array_to_string(v_film.location, ', '), '');

  v_combined_content := concat_ws(
    ' ',
    nullif(trim(v_film.name), ''),
    nullif(trim(v_film.teaser), ''),
    nullif(trim(v_film.description), ''),
    nullif(trim(v_film.note), ''),
    nullif(trim(v_film.story_date), ''),
    v_location,
    v_creators,
    v_rating_name,
    v_theme_name,
    v_tags,
    'film',
    v_transcript
  );

  insert into public.search (
    internal_id,
    item_type,
    slug,
    name,
    teaser,
    blok_tall,
    search_content,
    release_date,
    rating,
    theme,
    runtime,
    creator,
    label
  ) values (
    v_film.id,
    'film',
    v_film.slug,
    v_film.name,
    v_film.teaser,
    v_film.blok_tall,
    v_combined_content,
    v_film.release_date::date,
    v_rating_name,
    v_theme_name,
    v_film.runtime,
    v_creators,
    null
  )
  on conflict (internal_id) do update set
    item_type = excluded.item_type,
    slug = excluded.slug,
    name = excluded.name,
    teaser = excluded.teaser,
    blok_tall = excluded.blok_tall,
    search_content = excluded.search_content,
    release_date = excluded.release_date,
    rating = excluded.rating,
    theme = excluded.theme,
    runtime = excluded.runtime,
    creator = excluded.creator,
    label = excluded.label,
    updated_at = now();
end;
$$;

-- Keep search fresh when credits / transcripts change
create or replace function public.tr_sync_film_search_from_child()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
declare
  v_film_id uuid;
begin
  v_film_id := coalesce(new.film_id, old.film_id);
  if v_film_id is not null then
    perform public.sync_film_to_search(v_film_id);
  end if;
  return coalesce(new, old);
end;
$$;

drop trigger if exists tr_creator_map_film_search on public.creator_map;
create trigger tr_creator_map_film_search
after insert or update or delete on public.creator_map
for each row
execute function public.tr_sync_film_search_from_child();

drop trigger if exists tr_transcript_film_search on public.transcript;
create trigger tr_transcript_film_search
after insert or update or delete on public.transcript
for each row
execute function public.tr_sync_film_search_from_child();

-- Rebuild all film search rows with the improved payload
do $$
declare
  r record;
begin
  for r in select id from public.film loop
    perform public.sync_film_to_search(r.id);
  end loop;
end;
$$;
