...
export default function WatchPage() {
  ...
  return (
    <div className="fixed inset-0 w-full h-[100svh] bg-[#1f1f1f] text-[#F5F5F7] select-none overflow-hidden flex flex-col justify-between font-sans">
      ...
      >
        <div className="flex items-center justify-start">
          <svg 
            ...
            className="h-5 w-auto text-white/70"
          >
      ...
      {/* Centering wrapper remains clean and holds no internal panel decorations */}
      <div className="flex-grow w-full flex items-center justify-center">
        
        {/* 🎬 MAIN CONTAINER BOX */}
        {/* Strip out layout actions from the bottom here so it fits edge-to-edge independently */}
      ...
      {/* 🎯 TRANSFERRED AND ADAPTED FOR SAFARI SAFE-AREAS:
          Moved control systems down out of the central video component. Pinned with `fixed bottom-0`.
          The `pb-[calc(env(safe-area-inset-bottom)+1.5rem)]` layout string safeguards button hits from overlapping iOS Safari parameters. */}
      <div 
        className="fixed bottom-0 inset-x-0 w-full flex flex-col justify-end items-start z-30 transition-opacity duration-500 select-none px-8 pb-[calc(env(safe-area-inset-bottom)+1.5rem)] gap-3 bg-gradient-to-t from-black/60 via-black/20 to-transparent"
        style={{ opacity: controlsVisible ? 1 : 0, pointerEvents: controlsVisible ? 'auto' : 'none' }}
      >
        ...
        <div className="w-full flex items-center justify-between gap-4 h-5 pl-1 mt-1 relative">
