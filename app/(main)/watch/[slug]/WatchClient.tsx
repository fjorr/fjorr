    ...
    <div className="fixed inset-0 w-full h-[100svh] bg-[#1f1f1f] text-[#F5F5F7] select-none overflow-hidden flex flex-col justify-between font-sans">
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "Movie",
            "name": film?.name,
            "description": `Stream and watch ${film?.name} on Fjorr.`,
            "image": film?.blok_ogrf || "https://fjorr.com/og-main-preview.jpg",
            "potentialAction": {
              "@type": "WatchAction",
              "target": `https://fjorr.com/watch/${film?.slug}`
            }
          })
        }}
      />
      ...
      >
        ...
        <div className="flex items-center gap-2 h-13 relative">
          <button 
            onClick={togglePlay} 
            className="w-12 h-12 flex items-center justify-center opacity-70 hover:opacity-100 transition-opacity duration-200 cursor-pointer"
          ...
          >
            <img src="/icons/forward10.svg" className="w-6 h-6 invert" alt="Fast Forward 10s" />
          </button>

          <button 
            onClick={() => setShowCCMenu(!showCCMenu)} 
            className={`w-10 h-10 flex items-center justify-center transition-opacity duration-200 cursor-pointer ${showCCMenu ? 'opacity-100' : 'opacity-70 hover:opacity-100'}`}
            title="Captions"
          >
            <img 
              src="/icons/cc.svg" 
