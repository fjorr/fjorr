...
export default function AboutClient() {
  ...
  const manifestoLines = t('manifesto').split('\n');
  ...
  useEffect(() => {
    ...
    const initScrollTimeline = () => {
      ...
      // 🎥 PART 1: Main Video Playback Scrubbing (skip if media missing)
    ...
    const onLoadedMetadata = () => initScrollTimeline();
    ...
    const runTimers = setTimeout(() => {
      if (!video) {
        ...
        if (video.readyState >= 1) {
          ...
          // If the video 404s / never loads, still reveal the page.
          ...
          <div ref={heroSectionRef} className="absolute inset-0 flex flex-col items-center justify-center text-center z-20 px-6 max-w-4xl mx-auto pointer-events-none">
            <h1 className="font-futura text-[80px] sm:text-[80px] md:text-[100px] lg:text-[120px] leading-[0.75em] font-black uppercase tracking-tighter text-[#f5f5f7] w-full">
              <div ref={titleLine1Ref} className="flex flex-col">
                <span className="reveal-line block opacity-0 translate-y-7">{t('heroThe')}</span>
                <span className="reveal-line block opacity-0 translate-y-7">{t('heroWorld')}</span>
                <span className="reveal-line block opacity-0 translate-y-7">{t('heroRunsOn')}</span>
                <span className="reveal-line block text-[#f5f5f7] opacity-0 translate-y-7">{t('heroAtoms')}</span>
              </div>
              <span ref={titleLine2Ref} className="reveal-line block text-[#f5f5f7] mt-0 opacity-0 translate-y-7">{t('heroAnd')}</span>
              <span className="reveal-line block text-[#f5f5f7] mt-0 opacity-0 translate-y-7">{t('heroStories')}</span>
            </h1>
          </div>
...
<div className="w-[250px] h-[250px] relative flex items-center justify-center">
  ...
  <div ref={helmetTextRef} className="mt-6 text-center font-mono text-base tracking-tight text-[#f5f5f7]/80 opacity-0 max-w-sm">
    <div className="font-bold border border-white/60 rounded px-2 py-0.5 text-[11px] uppercase tracking-normal text-white/60 mb-3 w-max mx-auto">{t('logoLabel')}</div>
    <span className="text-white text-lg font-bold block mb-1">{t('logoTitle')}</span>
    {t('logoBody')}
  </div>
</div>
          ...
          <div ref={wordmarkStageRef} className="absolute inset-0 flex flex-col items-center justify-center p-6 opacity-0">
            ...
            <div ref={wordmarkTextRef} className="mt-8 text-center font-mono text-base tracking-tight text-[#f5f5f7]/70 opacity-0 px-6 max-w-sm">
              <div className="border border-white/60 rounded px-2 py-0.5 text-[11px] font-semibold uppercase tracking-normal text-white/60 mb-3 w-max mx-auto">{t('nameLabel')}</div>
              <span className="text-white font-bold block text-lg mb-1">{t('nameTitle')}</span>
              {t('nameBody')} <br/>
              <span className="text-sm text-white/60 mt-2 block italic">{t('namePronunciation')}</span>
            </div>
          </div>
