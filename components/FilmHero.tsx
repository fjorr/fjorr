...
interface FilmHeroProps {
  film: any;
  onPlayClick: () => void; // 🎯 DIRECT ACTION PROPERTY LINK CAPTURE
}
      ...
      <div className="w-full relative overflow-hidden rounded-none shadow-2xl">
        <div className="w-full block aspect-[1/1.618] md:aspect-[4/3] lg:aspect-[16/9] flex flex-col justify-end px-8 md:px-12 pb-8 md:pb-12 pt-[220px] relative bg-cover bg-center transition-all duration-500 select-none" style={{ backgroundImage: `linear-gradient(to top, rgba(0,0,0,0.95) 0%, rgba(0,0,0,0.4) 50%, rgba(0,0,0,0) 100%)` }}>
          ...
          <div className="relative z-20 max-w-2xl w-full flex flex-col items-center text-center md:items-start md:text-left mx-auto md:mx-0 mt-auto">
            {film.sponsor && <div className="w-full font-sans font-bold text-[13px] text-white/90 tracking-wide mb-2.5 antialiased">{film.sponsor} <span className="text-white/50">presents</span></div>}

            {film.title_art_code ? (
              <div className="mb-4 max-w-[220px] md:max-w-[280px] w-full [&>svg]:w-full [&>svg]:h-auto mx-auto md:mx-0 transition-transform duration-300" style={{ color: film.title_art_hex || '#FFFFFF' }} dangerouslySetInnerHTML={{ __html: film.title_art_code }} />
            ) : (
            ...
            <div className="flex items-center justify-center md:justify-start gap-2.5 font-mono text-sm text-white/60 tracking-normal mb-2 select-none">
              {(() => { const ratingVal = typeof film.rating === 'object' ? film.rating?.name : film.rating; if (!ratingVal) return null; return <span className="px-1 py-0.25 border border-white/70 rounded-[4px] text-white/70 font-extrabold text-[12px] uppercase">{ratingVal}</span>; })()}
              {(() => { const themeVal = typeof film.theme === 'object' ? film.theme?.name : film.theme; if (!themeVal) return null; return <span className="capitalize font-semibold text-white/70"> {themeVal}</span>; })()}
            </div>
