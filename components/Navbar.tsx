...
function Navbar({ variant = 'light' }: NavbarProps) {
  const [isTheaterOpen, setIsTheaterOpen] = useState(false); // 🎯 STATE CONTROLLER LINK
  const textColor = variant === 'light' ? 'text-white' : 'text-black';
  const subTextColor = variant === 'light' ? 'text-white/80' : 'text-black/80';

  // 🎯 SECURED EVENT HANDSHAKE LISTENER: Catches cinema player toggle actions globally
  useEffect(() => {
    const handleHide = () => setIsTheaterOpen(true);
  ...
  // 🔌 Completely drop the navbar structure from the DOM when theater view layout is active
  ...
  return (
    ...
    <header className="sticky top-0 z-50 w-full h-[70px] pt-[20px] px-4 flex justify-center pointer-events-none">
      ...
      >
        ...
        {/* Right: Search Icon Vector */}
        <Link href="/search" className={`w-[18px] h-[18px] flex items-center justify-center cursor-pointer shrink-0 transition-opacity hover:opacity-70 ${textColor}`}>
          <Search size={18} color="currentColor" strokeWidth={2.5} />
        </Link>
