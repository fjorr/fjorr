...
function Navbar({ variant = 'light' }: NavbarProps) {
  ...
  return (
    // Parent Container: Fixed 70px height bounding box, sticky, centered layout
    <header className="sticky top-0 z-50 w-full h-[70px] pt-[20px] px-4 flex justify-center pointer-events-none">
      
      {/* Floating Center Container: 50px high, sharp 10px rounded corners.
        Using `inline-flex` with a strict `gap-[20px]` to keep the entire block tight and compact.
      */}
      <div
      ...
      >
        {/* Left: Official Fjorr Logo Box (Scaled precisely to your 50px width requirement) */}
        ...
        {/* Center: Slogan Display (12px Inter Medium, links to about page) */}
        <Link 
        href="/about" 
        ...
        {/* Right: Search Icon Vector (Strict 18x18px bounding box and matching adaptive fill color) */}
        <Link 
          href="/search" 
          className={`w-[18px] h-[18px] flex items-center justify-center cursor-pointer shrink-0 transition-opacity hover:opacity-70 ${textColor}`}
        >
          <Search 
            size={18} 
            color="currentColor" 
