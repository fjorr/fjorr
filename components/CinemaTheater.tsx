...
import TheaterRamsChrome from '@/components/TheaterRamsChrome';
    ...
    >
      {isRamsChrome ? (
        <button
          ...
          className={`absolute top-[max(1rem,env(safe-area-inset-top))] right-[max(1rem,env(safe-area-inset-right))] z-50 w-8 h-8 flex items-center justify-center cursor-pointer text-white/70 hover:text-white bg-transparent border-0 p-0 outline-none transition-all duration-500 ease-out ${
            controlsVisible
      ...
      <div
        className={
          isRamsChrome
            ...
            : 'flex-1 min-h-0 w-full flex items-center justify-center relative z-10'
        }
      >
        <div
          className={`relative overflow-hidden bg-black z-10 flex flex-col justify-end ${
            isFullscreen
              ...
              : isEmbed
                ...
                : isRamsChrome
                  ? 'w-full max-w-[1200px] aspect-video max-h-[calc(100dvh-10rem)] rounded-none min-[1201px]:rounded-[12px] pointer-events-auto'
        ...
        >
          ...
          {!isRamsChrome && selectedLangCode !== 'none' && currentSubtitleText && (
            <div
      ...
      {isRamsChrome && !isPlayingLogo && (
        <div
          className={`absolute bottom-[max(1.5rem,env(safe-area-inset-bottom))] left-1/2 -translate-x-1/2 z-40 transition-all duration-500 ease-out ${
            controlsVisible
              ? 'translate-y-0 opacity-100'
      ...
      >
        <div className="max-w-2xl text-center flex flex-col items-center gap-8 px-6 relative">
          ...
          <div
            className={`flex items-center gap-6 font-sans font-semibold text-sm ${
              isLight ? 'text-[#0B0B0C]/50' : 'text-white/50'
            }`}
          >
            <button
              onClick={handleCloseNavigation}
              className={`flex items-center gap-2 transition-colors group bg-transparent border-0 outline-none cursor-pointer font-sans ${
                isLight ? 'hover:text-[#0B0B0C]' : 'hover:text-[#f5f5f7]'
              }`}
            >
              {isEmbed ? t('watchOnFjorr') : t('close')}
            </button>
            <button
              onClick={() => setSendOpen(true)}
              className={`flex items-center gap-2 transition-colors group bg-transparent border-0 outline-none cursor-pointer font-sans normal-case ${
                isLight ? 'hover:text-[#0B0B0C]' : 'hover:text-[#f5f5f7]'
              }`}
            >
              {t('share')}
